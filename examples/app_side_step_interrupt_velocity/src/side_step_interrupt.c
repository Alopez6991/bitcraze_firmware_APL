/**
 * RC-switch triggered: ARM -> TAKEOFF -> settle ->
 *   LEFT sidestep -> FORWARD (with spike guard) -> RIGHT sidestep (stop on spike) ->
 *   HOVER/creep-right -> LAND -> DISARM
 *
 ...
 * - Down-range spike detector (mm) for terrain-rise/edge
...
// Lateral sidestep
#define SIDE_DISTANCE_M           0.75f       // sidestep distance (m)
#define SIDE_VEL_MPS              0.25f       // sidestep speed (m/s)
...
// Down-range spike guard (units mm)
#define STEP_THRESH_MM            120.0f      // step change threshold (mm)
#define RATE_THRESH_MM_PER_S      300.0f      // rate threshold (mm/s)
#define ENABLE_SPIKE_NEAR_DIST_M  1.00f       // only arm spike stop when this close to target

// Post-final-right creep
#define CREEP_VEL_MPS             0.15f
#define CREEP_TIME_MS             600U
...
/*********** Forward segment (returns true if stopped by spike) ***********/
static bool runWorldVelToDistance(float baseVx, float targetDist)
{
  bool spiked = runVelAlongDir(baseVx, targetDist, /*dirX*/+1.0f, /*dirY*/0.0f, /*stopOnSpike*/true);
  // brief hover after stopping
...
{
  DEBUG_PRINT("Sequence: ARM -> TAKEOFF -> settle -> left -> forward -> right(stop-on-spike) -> HOVER/creep-right -> LAND -> DISARM\n");
  ...
  // 1) Lateral LEFT (Y+), detect spikes but do not stop
  (void)runVelAlongDir(/*speed*/SIDE_VEL_MPS, /*dist*/SIDE_DISTANCE_M, /*dirX*/0.0f, /*dirY*/+1.0f, /*stopOnSpike*/false);
  holdZ_ms(TARGET_HEIGHT_M, 300U);

  // 2) Forward segment with spike guard
  const bool spikedForward = runWorldVelToDistance(BASE_FWD_VEL, TARGET_DISTANCE_M);

  // 3) If the forward segment spiked, skip the final right move
  if (!spikedForward) {
    // Lateral RIGHT (Y-), detect and stop on spike
    (void)runVelAlongDir(/*speed*/SIDE_VEL_MPS, /*dist*/SIDE_DISTANCE_M, /*dirX*/0.0f, /*dirY*/-1.0f, /*stopOnSpike*/true);
    holdZ_ms(TARGET_HEIGHT_M, HOVER_TIME_MS);

    // Creep to the RIGHT (Y-) like the forward-segment creep but lateral
    feedHover_ms(/*vx*/0.0f, /*vy*/-CREEP_VEL_MPS, TARGET_HEIGHT_M, CREEP_TIME_MS);
    zeroBurst_ms(/*ms*/300, /*hz*/100);
  }
