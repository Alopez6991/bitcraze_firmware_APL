...
// Emergency checker: if distance0 > threshold with hysteresis, latch abort
...
void appMain(void) {
  DEBUG_PRINT("drone_2_task: ranging.aux1 triggers; abort if ranging.distance0 > %u mm\n", DIST0_ABORT_MM);

  // Resolve required log IDs
  while (!logVarIdIsValid(idRangingAux1) || !logVarIdIsValid(idDistance0)) {
    ensureLogId(&idRangingAux1, "ranging", "aux1");
    ensureLogId(&idDistance0,   "ranging", "distance0");
